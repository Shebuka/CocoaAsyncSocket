...
- (instancetype)initWithDelegate:(<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
                        ...
                        port:(uint16_t)port
{
	...
	{
		NSString *temp = [components firstObject];
		if ([temp length] > 0)
		{
			interface = temp;
		}
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			currentRead = [readQueue firstObject];
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		...
		{
			// Dequeue the next object in the write queue
			currentWrite = [writeQueue firstObject];
